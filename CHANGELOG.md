...
### Major Changes
- damage from a pet whose name the game stopped writing is no longer counted as yours. When a hangar pet is destroyed its shots keep landing for another second, and the game writes those lines with the source field simply blank — which used to read as "the player fired this", so a weapon you do not even carry showed up as a row of your own damage. Those lines now go to the pet, worked out from what the rest of the fight says about that same weapon. Where several pets could have fired it, the damage is shown in a row called **(Damage owner unknown)** rather than being guessed at or quietly added to yours. An effect you applied and something else set off — Kemocite, Khitomer Shield, Mycelial Lightning — stays yours, because those you really do fire
- damage that arrives with nobody named as its source is now listed under the name of what hit you. An enemy torpedo already in flight when its launcher died writes a line the game leaves unsigned; those hits used to pile up in Damage Taken under a row called "unknown". The row now carries the name of the torpedo, or whatever else landed, which is the only thing such a line says about where it came from
- the damage your pets do to shields is now counted as theirs. The game writes a shot as two lines — one for the shields, one for the hull — and for hangar pets it names the pet on only one of them, so half of every shot they fired was being counted as yours. That is how a weapon you do not even carry could appear as a row of your own damage. Both halves now go to whoever fired the shot, so a pet's row finally shows its full damage. Where the log genuinely cannot say who fired — a shot with no hull line, or one that several sources arrived on at once — nothing is guessed and the figure stays where it was
- your pets' damage no longer lands in the row named after your own weapon. A grouping rule of yours folds effects together, but it used to fold the shooter in with them: if a pet carried the same turret or torpedo as your build, its shots were counted inside your row for that weapon, while the shots whose names happened not to match the rule stayed under the pet — so the same pet sat in two places in the tree and neither figure was really anybody's. Pets now keep everything they fired under their own row, your rules still fold their effects there, and your row holds what you fired. On a real Infected: The Conduit run this moved 15.7% of the outgoing damage back where it came from. If you would rather see one effect gathered from every source at once, that is what a Source Reversal rule has always been for
...
- a rule can now be tried against a real fight while you write it. Press the ✏ on any rule in Settings → Analysis and it opens in a window in the middle of the screen, with the names from the fight you have selected listed beside its conditions — updating as you type, so widening a pattern until the right names appear and no others is the whole job. Each heading counts what it looked at, so "0 of 340" tells you the rule catches nothing rather than leaving you to wonder whether it ran at all, and any name in the list can be clicked to copy it. The old box under the ✏ opened wherever there happened to be room and shut itself the moment you touched one of its own drop-downs
- where two of your grouping rules catch the same effect, the more precise one now takes it — whichever of them sits higher in the list. The rule that spells out more of the name wins, an exact match beats the same words used as a fragment, and a lone `*` loses to everything. So a rule for `Phaser Wide Angle Dual Heavy Beam Bank` takes those shots even though a broader `Phaser Wide Angle` rule is listed above it. Previously the higher rule simply won, which meant dragging a row could quietly change your numbers, and a rule copied from someone else worked or not depending on where you pasted it. If two rules fit an effect equally well, both rows get a ⚠, and a line under the table counts them and spells out the case of whichever rule you have picked — so you can see it without hunting for a tooltip. When nothing clashes it says that too. Checked on the maintainer's own 48 rules against a 138 MB log: nothing moved
- your rules can be exported to a file and imported back. Every tab in Settings → Analysis has its own **Export…** and **Import…**, and the pair at the top of the page covers all four at once — both write the same kind of file, so one exported from a tab can be imported at the top and the other way round. Import adds to what you already have and never clears it, a rule identical to one you have is skipped rather than duplicated, and a small window afterwards says how many were added and how many skipped. Nothing is saved until you press Ok, so Cancel undoes an import you did not mean
- your rules now live in their own file, `STO-CLARE_Rules.toml`, next to the settings. Copy that one file to another machine and your rules go with it — your log path, window size and handle stay behind. It is plain text laid out one rule to a paragraph, so you can open it in any editor and read what you have without picking your way through brackets. Nothing is asked of you on upgrade: the rules are moved there the first time the program starts, and your old settings file is kept beside the new one exactly as it was, so nothing you had is taken away

### Other Changes
- a working set of rules is published with STO-CLARE, in `rules/STO-CLARE_Rules.toml` in the repository. It is the maintainer's own: 48 custom grouping rules that give each weapon one row instead of one per firing mode, plus damage exclusions and source reversals. Bring it in with Import on the Analysis page — it adds to your own rules rather than replacing them, and most of the exclusions arrive switched off so nothing leaves your numbers without you ticking it
- your rules are listed alphabetically by name, so one can be found in a long list, and **the headings order the list** — click Group Name to turn the order round, or On to bring the switched-off rules to the top. The up and down arrows are gone: the order no longer decides anything. A rule you have just added stays at the end until you give it a name, and the list does not reshuffle while you are typing
- the last two columns of the rules tables finally have headings, so the warning mark and the bin say what they are
- rules can match with wildcards, in all four match methods. `*` and `%` both mean "anything at all here" and `?` means "exactly one character", so one condition — `Starts with` `Quad*Cannons` — stands for every firing mode of a weapon instead of one condition per name. The method still only says where your text has to sit; text with no wildcard in it works exactly as before, so none of your existing rules change
...
### Fixes
- a damaged settings or rules file no longer costs you anything. A file the program cannot read — a disk that filled up mid-write, an edit with one character wrong — used to be read as "nothing was ever set": everything came back at its default and the next Ok wrote those defaults over the file, taking your log path, your handle, your theme and your columns with it. STO-CLARE now puts the file aside under a name that says so, next to where it was, and starts as if it were newly installed. Nothing is deleted and nothing is written over: you can open the file, copy values back out of it, or repair it — and a repaired rules file goes straight back in with Import. The Settings window tells you which file it was, where it is now, and what the reader objected to. In the one case where the file cannot even be moved out of the way, the program leaves it completely alone and does not save at all until you have moved it yourself
