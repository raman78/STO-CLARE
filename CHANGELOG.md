...
- rules can match with wildcards. The new **Wildcard** method reads `*` and `%` as "anything at all here" and `?` as "exactly one character", so a single condition — `Quad*Cannons*` — stands for every firing mode of a weapon instead of one condition per name. Both `*` and `%` do the same thing, so whichever you are used to writing will work. The pattern has to cover the whole name, so put a `*` at each end if you mean "somewhere inside"
- the Group Name and Text to match boxes in the rules tables open wide enough to type into. A rule or condition just added by the ✚ button used to appear as a field a few characters across, and had to be widened by hand before it could be read. A longer name still widens the column past that. The Settings window can no longer be dragged narrower than the tables it holds
