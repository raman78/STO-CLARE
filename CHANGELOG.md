...
### Fixes
- a settings file that cannot be read no longer costs you your settings. A damaged file — a disk that filled up mid-write, an edit with one character wrong — used to be read as "no settings at all": everything came back at its default, the window looked like a fresh installation, and the next Ok wrote those defaults over the file, taking your log path, your handle, your theme and your chosen columns with it. The program now tells you at the top of the Settings window which file it could not read and what is wrong with it, works on defaults for that run, and leaves the file completely alone until you have repaired or moved it. Your rules file was already protected this way; now both are
