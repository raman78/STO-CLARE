...
- your rules now live in their own file, `STO-CLARE_Rules.json`, next to the settings. Copy that one file to another machine and your rules go with it — your log path, window size and handle stay behind. Nothing is asked of you on upgrade: the rules are moved there the first time the program starts. If the file ever cannot be read, the Analysis page says so and leaves the file alone rather than writing over it

### Other Changes
- a working set of rules is published with STO-CLARE, in `rules/STO-CLARE_Rules.json` in the repository. It is the maintainer's own: 48 custom grouping rules that give each weapon one row instead of one per firing mode, plus damage exclusions and source reversals. Bring it in with Import on the Analysis page — it adds to your own rules rather than replacing them, and most of the exclusions arrive switched off so nothing leaves your numbers without you ticking it
