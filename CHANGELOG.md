...
### Major Changes
- the combats list can be narrowed to the runs you flew something in. The new **Any damage** menu lists every weapon, ability, proc and pet that dealt damage in the fights on screen — the names off the Damage Dealt tab — with a tick box each, what you fly most at the top, and a search box for a log with a year of gear in it. Ticking two means both, so "the evening I ran the Krenim beams with the wide-angle torpedo" is two ticks. The **Search** box above the menus reads the same names, so typing `krenim` finds those runs without opening a menu at all. It counts what anyone in the fight fired, and lists only what did the shooting — never what it was shot at

### Other Changes
- the combats panel can be dragged out to three quarters of the window. It used to stop at a fixed width that was generous on a laptop and a cage on a wide screen, where there is room to have the whole of a long note and a team's players open beside the fights. The quarter left over is what keeps the tabs beside it readable, and a width set on a big screen is remembered whole — open the program on a smaller one and it comes back the next time there is room for it
...
- the note you write for a fight is now part of its name. It stands at the end of the line above the tabs, after a dash and in the same lettering as the name, so every tab is read under the run's own description and not only under the map it was flown on. The field itself has moved to its own line under that name — with a **Clear** button beside the counter, there while there is something to clear

### Fixes
- a fight ticked for a comparison stays ticked when you narrow the list. Ticking a run, then filtering or searching for the next one, used to quietly drop the first one — so a comparison built out of two searches ended up holding only the last fight ticked. The count at the bottom now says how many ticked fights are off screen. Clearing the log still works the other way round, on purpose: only what you can see can be deleted
