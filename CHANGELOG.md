...
### Other Changes
- **Esc** closes a dialog, the same way its Cancel or Close button does. Works in Settings, the rule-editing boxes, Delete combats, the occurred-names list, Damage by type, and the upload and download result windows. Pressing it while typing in a field leaves the field first and keeps the dialog open, so a half-written rule name is not thrown away by the key you press to get out of it. The windows that report work in progress — clearing the log, fetching a run — do not take it, because there the only thing to stop is the work itself
