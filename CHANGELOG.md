...
- a column heading is clickable across the whole of it, both of its lines, rather than only the strip with the word in it. The Name column orders the rows too, A to Z and back — in the main window's tables, in the Summary tab's player list and in a comparison. Looking for an ability you know the name of no longer means reading down a list ordered by size
...
- the list you pick a comparison from is a proper table now: the description you wrote for a run has a column of its own instead of being tacked onto the end of the combat's name, and clicking anywhere on a row picks that run, not only the tick box
...
- a comparison is read against a run you choose. It starts as the run with the best DPS — the natural thing to measure a set of attempts against — and the **Reference** line under the toolbar moves it to any other. The picked run leads the table, every difference beside a figure is against it, and **⚖ vs rest** measures it, so there is one answer to "compared to what" instead of two. A run keeps its own number and colour wherever it stands, so nothing on the chart or in the list changes colour when you move the reference
...
- the lists of runs — the one you pick a comparison from and the one inside it — are striped and light up under the pointer, the way the tables do. Picking the right line out of an evening's runs of the same map no longer means counting rows
...
- a chart no longer slides up and down when you turn the wheel over it. Its height always fits what it draws, so moving it vertically only pushed the lines out of view; dragging it sideways along the time axis still works
- the damage-type list no longer opens as a banner across the window. It is as wide as the words in it
- scroll bars sit at the edge of the panel they belong to instead of against the text. In Settings they were tucked behind the last column of a rules table or beside the longest map name, with an empty stretch of window to their right; the same went for the list of runs you pick a comparison from, the list of runs inside one, and the list of combats whose notes you can delete. In the list inside a comparison the bar could not be reached at all, because the line explaining the reference run ran past the right-hand edge and took the bar with it — that line is now cut off where the pane ends
