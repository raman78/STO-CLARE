...
| [Read a run from the ladder](MANUAL.md#reading-a-run-from-the-ladder) | Any run joins your combats list as a fight of its own, in a colour that says where it came from, and reads like one of yours — all the tabs, the charts, the ability breakdown. Your log is untouched, several runs can be open at once, and one button takes each back out again.        |
