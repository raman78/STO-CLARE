...
| Solo or team                                                                       | Every combat says whether you fought it alone — "[Solo] [Space] [TFO] Infected: The Conduit [Elite]" — and a menu keeps one kind or the other. The label follows the combat into a comparison, a summary pasted into chat and a saved file.                                                                         |
...
| [A note on every combat](MANUAL.md#describing-a-combat-so-you-can-find-it-again)   | Up to 50 characters written under a combat's name — they join that name in the heading over the tabs, and follow it through the combats list, the compare view and the chat summary.                                                                                                                                |
...
| [Read part of a build](MANUAL.md#reading-part-of-a-build)                          | A tick box on every ability row decides what the player's own figures are worked out from, healing included. Untick the torpedo and the DPS, resistance, critical rate and accuracy above are those of what is left, worked out again from the hits; the chart follows, and one button hides the rows you took out. |
| [One damage type at a time](MANUAL.md#one-damage-type-at-a-time)                   | The damage tabs can be narrowed to the types you pick, so "how much of this run was phaser" is one pick. Every figure is worked out for those types alone, which is not the same as reading the rows that carry them.                                                                                               |
| Tables you can order                                                               | Click any column heading and the table is in that order; the heading says so and carries an arrow you click again to turn it round. **Name** orders alphabetically, and All, Hull and Shield each order by their own figure.                                                                                        |
...
| Feature                                                                              | What it does                                                                                                                                                                                                                                                                                                                                 |
|--------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Compare Combats                                                                      | Tick fights in the combats list and read them side by side, the breakdown lined up group by group, with green and red numbers against the reference. Ticking one adds it there and then; each carries a number and colour the table and the chart use, and its own player picker.                                                                                               |
...
| [What one run did differently](MANUAL.md#what-one-run-did-differently-from-the-rest) | Pick a run and every row says how much DPS it added to it, or cost it, against what the other runs did — biggest first, and adding up exactly to the difference on the Total.                                                                                                                                                                |
| [The runs split by damage type](MANUAL.md#the-runs-split-by-damage-type)             | A window with one line per damage type and one column per run, so a rainbow build and a single-flavour one are told apart at a glance. Damage dealt to shields is counted with the weapon that dealt it, which the game itself does not record.                                                                                              |
| [One average instead of many columns](MANUAL.md#one-average-instead-of-many-columns) | A pile of runs can be read as one averaged column per metric. Damage and DPS are averaged over every run, so the ability rows still add up to the Total; the rates — critical, accuracy, resistance — count only the runs an ability was used in. A row missing from some runs says so beside its figure.                                     |
...
| [Read a run from the ladder](MANUAL.md#reading-a-run-from-the-ladder) | One press opens any run in the main window and joins it to your combats list, in a colour that says where it came from. It reads like one of yours — all the tabs, the charts, the ability breakdown. Your log is untouched, several runs can be open at once, and one button takes each back out again. |
| [Compare it with your own](MANUAL.md#comparing-it-with-your-own)      | A fetched run is a row of the list, so it is ticked into a comparison like any fight of yours. Opening one points the list's filters at the same map and level.                                                                                                                          |
...
|----------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------|
| [A table of your fights](MANUAL.md#the-combats-list)     | A panel down the side of the window lists every fight in the log — type, size, content, map, level, when it started, how long it ran, your DPS and your note — sortable by any of them, with each team fight foldable to show what every player did.       | —                                                                   |
| Narrow the list                                          | Search by name, note or what was fired in a fight, filter by type, level, map, solo or team, and narrow to when the fights were played. Each menu only offers what the others leave, so no combination shows nothing.                                      | —                                                                   |
| [Find the runs you flew something in](MANUAL.md#finding-the-runs-you-flew-something-in) | A menu of every weapon, ability, proc and pet that dealt damage in the fights on screen, most-flown first, with a tick box each — so an evening on one build can be found without having written a note about it. Tick two and you get the runs that fired both. | —                                                                   |
| Fights nobody died in                                    | Tick one player — you are at the top of the list — for the runs they came through alive, or several for the runs they all did. While you are clearing the log it turns round and finds the runs any of them died in, which is what you delete.             | —                                                                   |
| Choose what to delete                                    | "Clear Log File" turns the list into tick boxes, so you delete exactly the ones you mean to. Select all or none, everything but the newest is ticked for you, and anything past one fight asks first.                                                      | [#7](https://github.com/AnotherNathan/STO_CombatLogAnalyzer/pull/7) |
| A list that looks after itself                           | It fills in when the app starts and follows the log as you play — new fights appear without moving what you are reading. The first combat in a log can be saved and deleted like any other.                                                                | [#7](https://github.com/AnotherNathan/STO_CombatLogAnalyzer/pull/7) |
...
| [Keyboard shortcuts](MANUAL.md#keyboard-shortcuts) | The overlay, the Ladder, Settings, the grouping rules and a comparison are each one key away, and every one of them can be set to a combination of your own under Settings → Shortcuts, or taken off a key altogether when the game needs that combination. **Alt + O** is taken from the whole desktop, so the overlay comes and goes while the game is in front. | —                     |
